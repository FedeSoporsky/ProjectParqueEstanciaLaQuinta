...
public class GoogleStaticMap : MonoBehaviour
{
    ...
    public class MapRectangle
    {
        ...
        {
            var ratio = getHeightMeters() / getHeightLat_deg();
            return ratio;
        }
    ...
    {
        if (this._centerLatLon == null)
        {
            Debug.Log("centerLatLon is null");
        }

        float delta_lon = point.lon_d - this._centerLatLon.lon_d;
        //Debug.Log("delta_lon value: " + delta_lon);
        float delta_lat = point.lat_d - this._centerLatLon.lat_d;
        //Debug.Log("delta_lat value:" + delta_lat);
